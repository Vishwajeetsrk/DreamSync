...
import { Mail, Lock, User, Database, ArrowRight, ShieldCheck, AlertCircle, Sparkles, Zap, Globe, Eye, EyeOff, Loader2 } from 'lucide-react';
      ...
      >
        <div className="text-center space-y-6">
            ...
            <div className="space-y-2">
                <h1 className="text-3xl font-extrabold text-stone-900 tracking-tight">Create your account</h1>
                <p className="text-stone-400 font-medium">Join DreamSync and start your journey today.</p>
            </div>
        </div>
        ...
        <div className="bg-white p-8 md:p-12 rounded-[2.5rem] border border-stone-100 shadow-xl shadow-stone-200/50 space-y-10">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-4 bg-rose-50 border border-rose-100 rounded-2xl text-rose-600 text-sm font-bold flex items-start gap-3">
              ...
              <div className="space-y-3">
                ...
                {error.includes("Login instead") && (
                  <Link href="/login" className="inline-block px-4 py-2 bg-rose-100 text-rose-700 rounded-xl text-xs font-black uppercase hover:bg-rose-200 transition-colors">
                    Log in Here
                  </Link>
                )}
              </div>
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-stone-300 group-focus-within:text-blue-600 transition-colors" />
                <input
                  type="text"
                  required
            ...
            <div className="flex items-start gap-4 px-1 pt-2">
               <input type="checkbox" id="terms" required className="w-5 h-5 mt-1 border-stone-200 rounded-lg text-blue-600 focus:ring-blue-500 transition-all cursor-pointer" />
               <label htmlFor="terms" className="text-xs font-medium text-stone-400 leading-relaxed cursor-pointer">
                 By joining, you agree to our <Link href="/terms" className="text-blue-600 font-bold hover:underline">Friendship Terms</Link> and <Link href="/privacy" className="text-blue-600 font-bold hover:underline">Safety Policy</Link>.
               </label>
            </div>
          ...
          <div className="space-y-6 pt-4 border-t border-stone-50">
            <p className="text-center text-[10px] font-bold text-stone-300 uppercase tracking-widest">Or sign up with</p>
            ...
            <div className="text-center pt-2">
              <p className="text-sm font-medium text-stone-400">
                Already part of DreamSync? <Link href="/login" className="text-blue-600 font-extrabold hover:underline">Log in here</Link>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center items-center gap-6 text-stone-300">
          ...
          <span className="text-[10px] font-extrabold tracking-[0.3em] uppercase">Private & Secure Joining</span>
